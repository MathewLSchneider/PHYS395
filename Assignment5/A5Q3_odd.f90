! Hw5_1.f90 
! compile with: gfortran -O3 -fdefault-real-8 A5Q3_odd.f90 
...
do i=1,3
    E_limit1(i)= 0.0 +1.0*(i-1.0);
    E_limit2(i)= 1.0+1.0*(i-1.0);
end do

do i=4,8
    E_limit1(i)= -3.0 +2.0*(i-1.0);
    E_limit2(i)= -1.0 +2.0*(i-1.0);
end do

do i=9,10
    E_limit1(i)= -11.0 +3.0*(i-1.0);
    E_limit2(i)= -8.0 +3.0*(i-1.0);
end do
...
do j=2,10,2
        ...
        data(j,:)=integrate((-1.0)**(j-1.0), 5.0, 1e-3, E3, 2.0) 
end do 

write(*,*) "The eigenvalues for the odd solutions are...."
do i=2,10,2 
  write(*,*) EigenV(i)
end do 
...
function integrate(psi, t, dt, E, num)
  ...
  integer i, n
	...
	do i = 1,n
		call gl10(u, dt, E); if (abs(u(2)) > 100.0) exit
    if (num == 2) then 
...
function f(E, num, psi); intent(in) E, num
    real f, E, num, psi
    f = integrate(psi, 5.0, 1e-3, E, num) 
end function
