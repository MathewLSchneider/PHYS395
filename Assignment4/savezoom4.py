...
# plot image data
c = matplotlib.colors.LinearSegmentedColormap.from_list("difference", ["green", "red", "white"])
im = imshow(data, extent=(x[0],x[-1],y[0],y[-1]), origin='lower', aspect=(osx*dx)/(osy*dy), cmap=plt.get_cmap('afmhot'), interpolation='none')
...
savefig('Double_Pendulum_Chaos_Zoom4.pdf')